...
export default function Tab({ name }: { name: IconsType }) {
	if (name === "paymentsSolid") {
		return (
			<button
				...
				aria-label={`${name}`}
				className='leading-5 selected group flex px-4 py-2 gap-3 cursor-pointer w-full rounded-md hover:bg-hoverBg'>
	...
	return (
		...
		<button
			aria-label={`${name}`}
			className='leading-5 flex px-4 py-2 gap-3 cursor-pointer w-full rounded-md hover:bg-hoverBg'>
